...
export const FoodVendors: React.FC = () => {
  ...
  const { items: vendors, loading, error } = useRealtimeCollection<FoodVendorRecord>('vendors', (raw, id) => ({
    id,
  ...
  const filteredVendors = useMemo(
    () =>
      vendors
        .filter((vendor) => vendor.vendorType.toLowerCase().includes('food') || vendor.vendorId.toLowerCase().startsWith('food'))
  ...
  return (
    <div className={pageShellClassName}>
      <AdminPageHeader
        ...
        title="Food Vendors"
        description="A realtime list of food-side vendors with a cleaner review flow on both desktop and mobile screens."
      />

      <div className="space-y-6 p-4 sm:p-6">
        {loading ? <LoadingPulse label="Subscribing to food vendors..." /> : null}

        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <AdminStatCard label="Food Vendors" value={stats.total.toString()} meta="All matched food vendor records" accent="orange" />
        ...
        {error ? (
          <AdminEmptyState title="Food vendors could not be loaded" description="The realtime vendor listener failed for the vendors collection." />
        ) : filteredVendors.length === 0 && !loading ? (
          <AdminEmptyState title="No food vendors match the filters" description="Try clearing the search or reviewing the vendor typing data in Firestore." />
        ) : (
          <AdminPanel title="Food Vendor Queue" subtitle="Responsive list of live food-side vendor documents">
                ...
                <tbody className="divide-y divide-slate-800 bg-slate-950/40">
                  {filteredVendors.map((vendor) => (
                    <tr key={vendor.id} className="transition hover:bg-slate-900/80">
                      ...
                      <td className="px-4 py-4">
                        <button className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-3 py-2 text-xs font-semibold text-white transition hover:bg-orange-400" onClick={() => navigate(`/admin/vendors/${vendor.id}`)}>
                          View
