...
export const FoodVendors: React.FC = () => {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      ...
      allVendors = allVendors.filter(v => v.type === 'food');
  ...
  return (
    <div className="bg-white rounded-xl shadow p-6">
      <h2 className="text-2xl font-bold mb-6 text-orange-600">Food Vendors</h2>
      {/* Filter Card */}
      ...
      {loading ? <div>Loading...</div> : (
        <div className="overflow-x-auto">
          <table className="min-w-full border rounded-xl overflow-hidden">
            ...
            <tbody>
              {vendors.map(vendor => (
                <tr key={vendor.id} className="hover:bg-orange-50 cursor-pointer">
                  ...
                  <td className="border px-4 py-2 flex gap-2">
                    <button className="bg-orange-500 text-white px-3 py-1 rounded text-xs" onClick={() => navigate(`/admin/vendors/${vendor.id}`)}>View</button>
                  </td>
                </tr>
              ))}
            </tbody>
