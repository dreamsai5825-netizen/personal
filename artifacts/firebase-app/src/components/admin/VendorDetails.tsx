  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      <div className="bg-white rounded-xl shadow-lg p-8 mb-6">
        <h2 className="text-2xl font-bold mb-6 text-green-600">Vendor Details</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <DetailCard label="Vendor ID" value={vendor.vendorId || vendorId} />
          <DetailCard label="Name" value={vendor.name} />
          <DetailCard label="Email" value={vendor.email} />
          <DetailCard label="Phone" value={vendor.phone} />
          <DetailCard label="Status" value={vendorStatusOptions.find(s => s.value === vendor.status)?.label || vendor.status} />
          <DetailCard label="Registered At" value={vendor.createdAt ? vendor.createdAt.replace('T', ' ').slice(0, 16) : ''} />
        </div>
        <div className="flex flex-wrap gap-4 mt-6">
          <button className="bg-green-500 text-white px-4 py-2 rounded" onClick={() => handleAction('Activate')}>Activate</button>
