  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      <div className="bg-white rounded-xl shadow-lg p-8 mb-6">
        <h2 className="text-2xl font-bold mb-6 text-blue-600">Driver Details</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <DetailCard label="Driver ID" value={driver.driverId || driverId} />
          <DetailCard label="Name" value={driver.name} />
          <DetailCard label="Email" value={driver.email} />
          <DetailCard label="Phone" value={driver.phone} />
          <DetailCard label="Status" value={driverStatusOptions.find(s => s.value === driver.status)?.label || driver.status} />
          <DetailCard label="Registered At" value={driver.createdAt ? driver.createdAt.replace('T', ' ').slice(0, 16) : ''} />
        </div>
